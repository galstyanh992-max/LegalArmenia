...
async function run() {
  ...
  for (const role of roles) {
    const email = `${role}.main@app.internal`; // from output earlier: admin.main@app.internal
    const altEmail = `${role}_user@app.internal`;
    const password = Math.random().toString(36).slice(-8) + 'Aa1!';
    
    // Find the user by email
    const { data: usersData, error: fetchError } = await supabase.auth.admin.listUsers();
    const targetUser = usersData.users.find(u => u.email === email || u.email === altEmail);
    ...
    if (targetUser) {
      ...
      if (updateError) {
        ...
        credentials.push({ role, email: targetUser.email, password });
      }
    } else {
