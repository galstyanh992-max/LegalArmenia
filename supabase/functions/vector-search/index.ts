...
import { deriveSemanticStatus } from "../_shared/semantic-status.ts";
...
serve(async (req) => {
  ...
  try {
    ...
    const semanticStatus = deriveSemanticStatus({
      embeddingAttempted: Boolean(metricEmbedding),
      hasSemanticRows,
      hasKeywordRows,
    });
    const semanticOk = semanticStatus.semantic_ok;
    ...
    return json({
      ...
      semantic_attempted: semanticStatus.semantic_attempted,
      semantic_degraded: semanticStatus.semantic_degraded,
      fts_fallback_used: semanticStatus.fts_fallback_used,
