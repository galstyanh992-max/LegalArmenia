...
-- P0: Restore expired-lease recovery + add advisory lock to prevent concurrent orchestrator runs

-- 1. Fix claim_pipeline_jobs: re-add expired lease recovery
...
DECLARE
  claimed_ids uuid[];
BEGIN
  -- Select and lock candidates (includes expired leases), store their IDs
  ...
  FROM (
    ...
    WHERE pj.job_type = p_job_type
      AND (
        -- Normal pending jobs
        (pj.status = 'pending' AND (pj.next_run_at IS NULL OR pj.next_run_at <= now()))
        -- Expired leases: processing but lease expired → reclaim
        OR (pj.status = 'processing' AND pj.lease_expires_at IS NOT NULL AND pj.lease_expires_at < now())
      )
...
-- 2. Create a guard function for orchestrator concurrency (advisory lock)
...
AS $function$
BEGIN
  -- Advisory lock with a fixed key (hash of 'pipeline_orchestrator')
  -- pg_try_advisory_lock returns true if acquired, false if already held
  RETURN pg_try_advisory_lock(hashtext('pipeline_orchestrator'));
END;
$function$;

-- 3. Release function (called at end of orchestrator cycle)
...
AS $function$
BEGIN
  PERFORM pg_advisory_unlock(hashtext('pipeline_orchestrator'));
END;
$function$;
