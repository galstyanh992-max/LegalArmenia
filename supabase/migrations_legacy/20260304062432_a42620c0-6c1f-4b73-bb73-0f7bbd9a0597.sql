-- Update RPCs to check for current chunker version (v2.4.3+)
-- Docs needing rechunk = those without any chunk matching 'v2.%' pattern
...
AS $$
  ...
  WHERE kb.is_active = true
    ...
    AND NOT EXISTS (
      ...
      WHERE c.kb_id = kb.id AND c.rechunk_version LIKE 'v2.%'
    )
  ORDER BY kb.id
  LIMIT p_limit;
$$;
...
AS $$
  ...
  WHERE lp.is_active = true
    ...
    AND NOT EXISTS (
      ...
      WHERE c.doc_id = lp.id AND c.rechunk_version LIKE 'v2.%'
    )
  ORDER BY lp.id
  LIMIT p_limit;
$$;